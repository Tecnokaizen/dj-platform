...
export default async function PlaylistDetailPage({
  ...
  const libraryItems = canManagePlaylists
  ...
  return (
    <section className="max-w-4xl">
      ...
      <p className="mt-2 text-sm text-neutral-500">
        {playlist.visibility} · {playlist.items.length} items
      </p>
            ...
            >
              ...
              {libraryItems.map((item) => (
                <option key={item.id} value={item.id}>
                  {item.track.title} — {formatArtistCredits(item.track.artists)}
                </option>
              ))}
            </select>
          </label>
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-neutral-400">
                    {formatArtistCredits(item.libraryItem.track.artists)} · BPM{' '}
                    {formatBpm(item.libraryItem.track.bpm)}
                  </p>
                </div>
                {canManagePlaylists ? (
